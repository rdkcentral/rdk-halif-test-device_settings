...
void test_l2_dsHdmiIn_GetNumberOfInputs_sink(void)
{
    ...
    UT_ASSERT_KVP_EQUAL_PROFILE_UINT8(pNumberOfinputs, "HdmiInputPort/numberOfPorts");
...
void test_l2_dsHdmiIn_GetStatus_source(void)
{
    ...
    UT_LOG_DEBUG("Verifying returned dsHdmiInStatus_t structure");
    UT_ASSERT_EQUAL(status.isPresented, false);
    for (int i = 0; i < dsHDMI_IN_PORT_MAX; i++)
    {
        UT_ASSERT_EQUAL(status.isPortConnected[i], false);
...
void test_l2_dsHdmiIn_VerifyHdmiInputPortStatus_sink(void)
{
    ...
    for (port = dsHDMI_IN_PORT_0; port < numInputs; port++)
    {
        ...
        UT_ASSERT_EQUAL(ret, dsERR_NONE);
        UT_ASSERT_FALSE(status.activePort);
...
void test_l2_dsHdmiIn_VerifyHdmiArcPort_sink(void)
{
    ...
    for(iPort = dsHDMI_IN_PORT_0; iPort < dsHDMI_IN_PORT_MAX; iPort++)
    {
        ...
        if(retStatus != dsERR_NONE)
        {
            UT_LOG_ERROR("dsIsHdmiARCPort failed with status: %d", retStatus);
            continue;
        }

        UT_ASSERT_KVP_EQUAL_PROFILE_UINT8(iPort, "HdmiArcPort/portNumber");
...
void test_l2_dsHdmiIn_SetAndGetEdidVersionAndValidateEdidLength_sink(void)
{
    ...
    tv_hdmi_edid_version_t getEdidVersion;
    ...
    for (int i = 0; i < 3; i++)
    {
        for (int j = 0; j < 2; j++)
        {
            ...
            UT_ASSERT_KVP_EQUAL_PROFILE_UINT32(length, "EDID_Data/edidbytesLength");
        }
    }
...
void test_l2_dsHdmiIn_GetSupportedGameFeaturesList_sink(void)
{
    ...
    UT_LOG_DEBUG("Return status: %d, gameFeatureCount: %d, gameFeatureList: %s", ret, features.gameFeatureCount, features.gameFeatureList);
    UT_ASSERT_EQUAL(ret, dsERR_NONE);

    UT_ASSERT_KVP_EQUAL_PROFILE_UINT32(features.gameFeatureCount, "gameFeatures/count");
...
void test_l2_dsHdmiIn_SetAndGetAllmSupport_sink(void)
{
    ...
    UT_LOG_DEBUG("Invoking dsSetEdid2AllmSupport with iHdmiPort set to dsHDMI_IN_PORT_0 and allmSupport set to true");
    ...
    UT_LOG_DEBUG("Invoking dsGetEdid2AllmSupport with iHdmiPort set to dsHDMI_IN_PORT_0");
    ...
    UT_LOG_DEBUG("Invoking dsSetEdid2AllmSupport with iHdmiPort set to dsHDMI_IN_PORT_0 and allmSupport set to false");
...
int test_dsHdmiIn_l2_register(void)
{
