...
void test_l2_dsDisplay_RetrieveAndValidateEDID_sink(void)
{
    ...
    // Manufacturer ID
    UT_LOG_INFO("Manufacturer ID: %d\n", edidBytes[8] << 8 | edidBytes[9]);
...
int test_l2_dsDisplay_register(void)
{
    ...
    pSuite1 = UT_add_suite("[L2 dsDisplay Source ]", NULL, NULL);
    if (pSuite == NULL)
    {
        return -1;
    }
    ...
    UT_add_test( pSuite1, "l2_dsDisplay_TestDefaultAspectRatio_source", test_l2_dsDisplay_TestDefaultAspectRatio_source);
