...
static int test_utils_parse_dsVideoPort()
{
    ...
    for(int i = 0; i < gDSvideoPort_NumberOfPorts; i++) {
        ...
        for(int j = 0; j < gDSVideoPortConfiguration[i].numSupportedResolutions; j++) {
            snprintf(key_string, TEST_UTIL_KVP_SIZE, "Ports/%d/supportedResolutons/%d/name" , i , j);
            ...
            snprintf(key_string, TEST_UTIL_KVP_SIZE, "Ports/%d/supportedResolutons/%d/pixelResolution" , i , j);
            ...
            snprintf(key_string, TEST_UTIL_KVP_SIZE, "Ports/%d/supportedResolutons/%d/aspectRatio" , i , j);
            ...
            snprintf(key_string, TEST_UTIL_KVP_SIZE, "Ports/%d/supportedResolutons/%d/stereoscopicMode" , i , j);
            ...
            snprintf(key_string, TEST_UTIL_KVP_SIZE, "Ports/%d/supportedResolutons/%d/frameRate" , i , j);
            gDSVideoPortConfiguration[i].supportedResolutons[j].frameRate = ut_kvp_getUInt32Field(ut_kvp_profile_getInstance(), key_string);
            snprintf(key_string, TEST_UTIL_KVP_SIZE, "Ports/%d/supportedResolutons/%d/interlaced" , i , j);
