...
*
* **Test Procedure:**
* Refer to UT specification documentation [ds-front-panel-display__L2_Low-Level_TestSpecification.md](../docs/pages/ds-front-panel-display__L2_Low-Level_TestSpecification.md)
*/
...
void test_l2_dsFPD_SetFPstateOFF_SetBrightness(void)
{
    ...
    for (int i=0;i<count;i++)
    {
        snprintf(buffer, DS_FPD_KEY_SIZE,"dsFPD/SupportedFPDIndicators/%d", i);
        eIndicator = ut_kvp_getUInt32Field( pInstance, buffer);
...
void test_l2_dsFPD_SetFPstateON_Multi_SetColor(void)
{
    ...
    for (int i=0;i<count;i++)
    {
        ...
        snprintf(colorbuffer, DS_FPD_KEY_SIZE, "dsFPD/%d/DEFAULT_COLOR_MODE",eIndicator);
        ...
        if (mode == 1)
        {
            ...
            snprintf(supportedColorbuffer, DS_FPD_KEY_SIZE, "dsFPD/%d/supportedColors",eIndicator);
            numOfSuppportedColors = ut_kvp_getListCount(pInstance,supportedColorbuffer);
            for (int j=0;j<numOfSuppportedColors;j++)
            {
                snprintf(buffer, DS_FPD_KEY_SIZE, "dsFPD/%d/supportedColors/%d", eIndicator, j);
