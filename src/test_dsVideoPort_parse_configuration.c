...
int test_dsVideoPort_parse_configuration()
{
    ...
    status = ut_kvp_getStringField(ut_kvp_profile_getInstance(), "dsVideoPort/Type", gDeviceType, DS_VIDEO_PORT_TYPE_SIZE);
    if (status == UT_KVP_STATUS_SUCCESS ) {
        if (!strncmp(gDeviceType, TEST_TYPE_SOURCE_VALUE, DS_VIDEO_PORT_TYPE_SIZE)) {
            gSourceType = 1;
        }
        else if(!strncmp(gDeviceType, TEST_TYPE_SINK_VALUE, DS_VIDEO_PORT_TYPE_SIZE)) {
            gSourceType = 0;
        }
        else {
