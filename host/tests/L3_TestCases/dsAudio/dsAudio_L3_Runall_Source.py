...
def Runall_L3(selected_tests=None):
    ...
    skipTests = ["test02", "test15", "test16", "test18", "test19", "test25", "test26"]

    # Summary log for all the tests
    dsAudioSummerLog = logModule("dsAudioSummerLog_Source", level=logModule.INFO)

    testDirectory = Path(dir_path)

    # Find all test modules in the directory
    test_modules = sorted(testDirectory.glob("dsAudio_test*.py"))
    ...
    for test_module_path in test_modules:
        ...
        try:
            ...
            test_instance = test_class(dsAudioSummerLog)
            test_instance.run(False)

        except (ImportError, AttributeError) as e:
            dsAudioSummerLog.error(f"Failed to import {module_name}: {e}")
