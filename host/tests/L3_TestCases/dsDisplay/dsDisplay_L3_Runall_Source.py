...
def Runall_L3():
    ...
    dsDisplaySummerLog = logModule("dsDisplaySummerLog_Source", level=logModule.INFO)

    testDirectory = Path(dir_path)

    # Find all test modules in the directory
    test_modules = sorted(testDirectory.glob("dsDisplay_test*.py"))
    ...
    for test_module_path in test_modules:
        ...
        try:
            ...
            test_instance = test_class(dsDisplaySummerLog)
            test_instance.run(False)

        except (ImportError, AttributeError) as e:
            dsDisplaySummerLog.error(f"Failed to import {module_name}: {e}")
