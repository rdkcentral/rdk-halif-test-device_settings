...
def Runall_L3(selected_tests=None):
    ...
    dsCompositeInSummerLog = logModule("dsCompositeInSummerLog_Sink", level=logModule.INFO)

    testDirectory = Path(dir_path)

    # Find all test modules in the directory
    test_modules = sorted(testDirectory.glob("dsCompositeIn_test*.py"))
    ...
    for test_module_path in test_modules:
        ...
        try:
            ...
            test_instance = test_class(dsCompositeInSummerLog)
            test_instance.run(False)

        except (ImportError, AttributeError) as e:
            dsCompositeInSummerLog.error(f"Failed to import {module_name}: {e}")
