...
def Runall_L3():
    ...
    dsHdmiInSummerLog = logModule("dsHdmiInSummerLog_Source", level=logModule.INFO)

    testDirectory = Path(dir_path)

    # Find all test modules in the directory
    test_modules = sorted(testDirectory.glob("dsHdmiIn_test*.py"))
    ...
    for test_module_path in test_modules:
        ...
        try:
            ...
            test_instance = test_class(dsHdmiInSummerLog)
            test_instance.run(False)

        except (ImportError, AttributeError) as e:
            dsHdmiInSummerLog.error(f"Failed to import {module_name}: {e}")
