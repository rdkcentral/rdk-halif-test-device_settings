...
def RunAll_L3_Source(selected_tests=None):
    ...
    dsVideoPortSummerLog = logModule("dsVideoPortSummerLog_Source", level=logModule.INFO)

    testDirectory = Path(dir_path)

    # Find all test modules in the directory
    test_modules = sorted(testDirectory.glob("dsVideoPort_test*.py"))
    ...
    for test_module_path in test_modules:
        ...
        try:
            ...
            test_instance = test_class(dsVideoPortSummerLog)
            test_instance.run(False)

        except (ImportError, AttributeError) as e:
            dsVideoPortSummerLog.error(f"Failed to import {module_name}: {e}")
