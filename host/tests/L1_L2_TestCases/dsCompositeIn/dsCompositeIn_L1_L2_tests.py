...
class dsCompositeIn_L1_L2_tests(utHelperClass):
    """
    Test class to run the L1 and L2 tests on the device.

    This class uses the `dsCompositeInClass` to interact with the device
    ...
    def __init__(self, log:logModule=None):
        ...
        testSetupPath = os.path.join(dir_path, "dsCompositeIn_L1_L2_testSetup.yml")
    ...
    def testFunction(self):
        ...
        for testsuite in testsuites:
            ...
            testdsCompositeIn = dsCompositeInClass(self.moduleConfigProfileFile, self.hal_session, testsuite_name, self.targetWorkspace, copyArtifacts)
            copyArtifacts = False
            ...
            if len(test_cases) == 1 and test_cases[0] == "all":
                ...
                result = testdsCompositeIn.runTest()
                if not result:
                    finalresult = False
...
if __name__ == '__main__':
    ...
    test = dsCompositeIn_L1_L2_tests(summeryLog)
    test.run(False)