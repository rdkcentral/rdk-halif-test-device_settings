...
class dsVideoPort_L1_L2_tests(utHelperClass):
    """
    Test class to run the L1 tests on the device.

    This class uses the `dsVideoPortClass` to interact with the device
    ...
    def __init__(self, log:logModule=None):
        ...
        testName = "dsVideoPort_L1_L2_Tests"
        qcId = "1"
        ...
        testSetupPath = os.path.join(dir_path, "dsVideoPort_L1_L2_testSetup.yml")
    ...
    def testFunction(self):
        ...
        for testsuite in testsuites:
            ...
            testdsVideoPort = dsVideoPortClass(self.moduleConfigProfileFile, self.hal_session, testsuite_name, self.targetWorkspace, copyArtifacts)
            copyArtifacts = False
            ...
            if len(test_cases) == 1 and test_cases[0] == "all":
                ...
                result = testdsVideoPort.runTest()
                if not result:
                    finalresult = False
                ...
                for test_case in testsuite.get("test_cases"):
                    ...
                    result = testdsVideoPort.runTest(test_case)
                    if not result:
                        finalresult = False
...
if __name__ == '__main__':
    ...
    test = dsVideoPort_L1_L2_tests(summeryLog)
    test.run(False)