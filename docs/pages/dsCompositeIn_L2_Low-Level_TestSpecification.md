...
### References
- `High Level Test Specification` - [dsComposite_test_spec.md](:/f89796b6c10046afacb25e551603a507)
...
|Description|Ensure that the function returns the expected number of COMPOSITE input ports by comparing the input port values parsed from the configuration YAML file 'Panel_CompositeInput.yaml'for the platform under test. The value to be retrieved from the YAML is 'composite_input_configurations/number_of_ports'.|
...
| -- | --------- | ---------- | -------------- | ----- |
| 01 | Initialize the COMPOSITE Input module using dsCompositeInInit() | None | dsERR_NONE | Should be successful |
| 02 | Loop through all the existing COMPOSITE Input ports on the platform based on the test profile "Panel_CompositeInput.yaml" and select each port using dsCompositeInSelectPort(port) | port = dsCOMPOSITE_IN_PORT_0 to max supported on platform and less dsCOMPOSITE_IN_PORT_MAX | dsERR_NONE | Should be successful |
