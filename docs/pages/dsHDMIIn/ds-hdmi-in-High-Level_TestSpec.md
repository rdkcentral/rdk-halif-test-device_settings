...
|19|[Set `EDID` to all `ALLM` Support](#set-edid-to-all-allm-support)|The test aims to verfiy by setting the `EDID` `ALLM` support.|
-----------
...
|Test Functionality|Description|HAL APIs|L2|L3|Source|Sink|Control plane requirements|
|------------------|-----------|--------|--|--|------|----|--------------------------|
|Set `EDID` to all `ALLM` Support|Set the `ALLM` support to `EDID` version 2.0 without connecting any source devices.|dsSetEdid2AllmSupport()|`Y`|`N`|`N`|`Y`|`NA`|
||Set `ALLM` support to `EDID` Version 2.0 with the connected external source device|dsSetEdid2AllmSupport()|`N`|`Y`|`N`|`Y`|`Y`|
